...
def warchest_state_copy(state: 'WarchestState') -> 'WarchestState':
    return WarchestState(
        ...
        enemy_fleets=list(state.enemy_fleets),        # copied: warchest_enemy_response appends to it
...
class Hellburner:
    ...
    def warchest_execute(
        ...
        for source_id, a in sorted(
            ...
            # Enemy response at the exact moment the source garrison is reduced: the
            # injected fleet launches now (state.turn == launch_turn) and can arrive
            # before latest_arrival, modelling an attack on the weakened source while
            # our fleet is still in transit.
            if self.PREDICT_ENEMY:
                self.warchest_enemy_response(state, horizon)
            total_ships    += a.ships
    ...
    def warchest_enemy_response(self, state: WarchestState, horizon: int) -> None:
        """Inject one greedy offensive move per enemy player into state.enemy_fleets.

        Each enemy player picks their highest-gain attack target (our planets or neutrals)
        using the same intercept mechanics as our own planner.  Ships are deducted from
        the source garrison immediately so the same ships cannot be injected again on the
        next DFS level.  Player -1 (neutral) never attacks and is always skipped.
        """
        enemy_players = {
            owner for owner in state.ownership.values()
            if owner not in (-1, self.player)
        }
        for enemy_player in enemy_players:
            already_inbound = {
                f.destination_id for f in state.enemy_fleets if f.owner == enemy_player
            }
            best_gain     = 0.0
            best_fleet: 'WarchestFleet | None' = None
            best_src_id   = -1
            best_fleet_sz = 0.0
            ...
            for target in self.planets:
                if target.id in self.comet_ids:
                    continue
                if state.ownership.get(target.id) == enemy_player:
                    continue
                if target.id in already_inbound:
                    continue
                prod_rate = (state.production.get(target.id, 0.0)
                             if state.ownership.get(target.id) != -1 else 0.0)

                for edge in self.inbound_edges.get(target, []):
                    src = edge.planet
                    if state.ownership.get(src.id) != enemy_player:
                        continue
                    ships = float(state.garrison.get(src.id, 0.0))
                    if ships <= 0:
                        continue
                    orb = self.orbital_info.get(src)
                    if orb is not None:
                        cx = cy = CENTER
                        a = orb.initial_angle + self.angular_velocity * state.turn
                        sx, sy = cx + orb.r * math.cos(a), cy + orb.r * math.sin(a)
                    else:
                        sx, sy = src.x, src.y
                    ic = self.intercept_planet(sx, sy, target, int(ships) or 1)
                    if not math.isfinite(ic.travel):
                        continue
                    arrival = state.turn + math.ceil(ic.travel)
                    if arrival > horizon:
                        continue
                    garrison_at_arrival = (state.garrison.get(target.id, 0.0)
                                           + prod_rate * math.ceil(ic.travel))
                    if ships <= garrison_at_arrival:
                        continue
                    gain = target.production * (horizon - arrival) - garrison_at_arrival
                    if gain <= best_gain:
                        continue
                    best_gain     = gain
                    best_src_id   = src.id
                    best_fleet_sz = ships
                    best_fleet    = WarchestFleet(
                        owner=enemy_player,
                        destination_id=target.id,
                        fleet_size=ships,
                        garrison_on_arrival=max(0.0, ships - garrison_at_arrival),
                        arrival_turn=arrival,
                        is_capture=True,
                        production_id=target.id,
                    )

            if best_fleet is not None:
                state.enemy_fleets.append(best_fleet)
                state.garrison[best_src_id] = max(
                    0.0, state.garrison.get(best_src_id, 0.0) - best_fleet_sz
                )
    ...
    def warchest_dfs(self, state, remaining, sequence, horizon, t0, best):
        ...
        for i, planet in enumerate(remaining):
            ...
            next_state = self.warchest_execute(warchest_state_copy(state), planet, assignment, horizon)

        if self.PREDICT_ENEMY:
            self.warchest_enemy_response(next_state, horizon)
